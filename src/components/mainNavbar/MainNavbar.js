...
export default function MainNavbar() {
    ...
    /* Navbar Animações do checkbox ao clicar (faz efeito de riscar a palavra de acordo com qual página está sendo acessada) */
    useEffect(() => {
        ...
        for(let i of ancors){
            ...
            i.onclick = function(){
                ...
                /* Limpa os outros ancors e deixa apenas setado o estilo no ancor atual */
                let ancorsInOtherScope = document.querySelectorAll('header nav a');

                for(let a of ancorsInOtherScope){ 
                    if(a.classList.contains("-ancorOfNavbarIsClicked")){
                        a.classList.remove("-ancorOfNavbarIsClicked");
                    }
                }

                /* Seta apenas no ancor corrente */
                this.classList.add("-ancorOfNavbarIsClicked");       
            }
        }
        ...
        if (window.location.href.substring(1).split("/")[3] == '' &&  window.location.href.substring(1).split("/").length == 4 || window.location.href.substring(1).split("/")[4] == '' && window.location.href.substring(1).split("/").length == 5) {
            ...
            // Não está no "/" e pode ter feito a chamada direto na URL, então devemos setar os estilos de acordo com ela!
            ...
            switch (window.location.href.substring(1).split("/")[4]) {
                ...
                case 'inputOutputLegacyAPI':
                    ancors[8].classList.add("-ancorOfNavbarIsClicked");
                    break;
                
                case 'newInputOutputAPI':
                    ancors[9].classList.add("-ancorOfNavbarIsClicked");
                    break;
                
                case 'serialization':
                    ancors[10].classList.add("-ancorOfNavbarIsClicked");
                    break;
            ...
            // OBS: Se a pessoa volta clicando pelas setas do navegador <= ou => o react mantém em cache o DOM Virtual
            // então ele reaproveita o que já foi carregado anteriormente, assim não executa o useEffect, portanto
            // não quis utilizar estados globais então decidi executar uma função em cada screen para atualizar a navbar!
            // Isso não é uma boa pratica, mas como quero realizar o deploy o mais rapido possivel e meu contexto agora
            // é Backend, vou fazer isso que é mais facil!
        }
